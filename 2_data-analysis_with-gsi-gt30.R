...
# set directory
dir_output <- "./output/with-gsi-gt30"

# DATA ----
## gonad ----
dir_data <- "./data"
data_obs <- read_rds(file.path(dir_data, "sol_gonads_2004_2023_sub_ready-for-analysis_with-gsi-gt30.rds"))
...
# model comparison
m1 <- lmer(log.gonad_weight ~ 1 + log.length + log.age + log.K_rel +
             (1 | week) + (1 | year), 
           data = data_sub,
           REML = F,
           na.action = "na.fail")

model_compare <- dredge(m1, subset = `log.length`)
# log.age is considered important -> retained in the model even thought model with log.age has delta AIC < 2

# best model
m1_reml <- lmer(log.gonad_weight ~ 1 + log.length + log.age + log.K_rel +
                  (1 | week) + (1 | year), 
                data = data_sub,
                REML = T)
...
# model comparison
m2 <- lmer(log.gonad_weight ~ 1 + log.length + log.age + log.K_rel +
             log.temp*log.length + log.temp*log.age + log.temp*log.ssb_lag1 + 
             (1 | week) + (1 | year), 
           data = data_sub,
           REML = F,
           na.action = "na.fail")

model_compare <- dredge(m2, subset = `log.length` & `log.age` & `log.K_rel`)

# best model
m2_reml <- lmer(log.gonad_weight ~ 1 + log.length + log.age + log.K_rel +
                  log.temp*log.length +
                  (1 | week) + (1 | year), 
                data = data_sub,
                REML = T)
